...
cp $INPUT_PATH/channel.ma2 .
cp $INPUT_PATH/channel.re2 .
...
do
	...
	for i_local_stat in {1..4}
	do
        printf -v N_FILE "%05d" $i_local_stat
		TEST_FILE=$INPUT_PATH/$i_folder/s01channel0.f$N_FILE
		...
		if [ -f "$TEST_FILE" ]; then
 			...
 			((i_total_snap_file++))
        		...
        		do
            		printf -v NS "%02d" $i_s
            		COPY_FROM0=$INPUT_PATH/$i_folder/s${NS}channel0.f$N_FILE
            		COPY_TO0=$OUTPUT_PATH/s${NS}channel0.f$LAST
        		...
        		do
            		printf -v NT "%02d" $i_t
            		COPY_FROM0=$INPUT_PATH/$i_folder/t${NT}channel0.f$N_FILE
            		COPY_TO0=$OUTPUT_PATH/t${NT}channel0.f$LAST
            		#echo "$COPY_FROM0 copied in $COPY_TO0"
            		cp $COPY_FROM0 $COPY_TO0
        		done
		else
